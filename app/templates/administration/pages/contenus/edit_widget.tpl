<div class="widget-header">
	...
	<h3>Modifier un contenu ({$content_type_label}) :</h3>
</div>
...
<div class="widget-content">
	<form id="meta-infos-form" action="index.php?p=informations_generales" method="post" class="form-horizontal">
        <div class="control-group">
            ...
            <div class="controls">
                <input class="input" id="content_title" name="content_title" type="text" value="{$content_title}" placeholder="Titre...">
            </div>
        </div>
        <div class="control-group">
            <label class="control-label" for="content_body">Corps de texte :</label>
            <div class="controls">
                <textarea id="content_body" name="content_body" rows="10" placeholder="Corps de texte...">{$content_body}</textarea>
            </div>
        </div>
				...
				<div class="controls">
					<select name="content_resource_id" id="content_resource_id">
						{foreach from=$list_resources key=id item=resource}
							<option value="{$id}" {if $picture_id == $id}selected="selected"{/if}>{$resource.title|capitalize}</option>
						{/foreach}
					</select>
				</div>
            </div>
